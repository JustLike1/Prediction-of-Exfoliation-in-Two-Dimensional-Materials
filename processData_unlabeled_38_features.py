...
# 数据处理流程说明（无标签版本 - 38个特征）：
# 1. 从指定文件夹中读取 max 和 min 子文件夹中的同名 .vasp 文件
...
def find_matching_files_unlabeled(split_dir, vasp_dir):
    """
    找到文件夹中 max 和 min 子文件夹里的匹配文件对（无标签版本）
    
    参数:
        split_dir: 包含 max 和 min 子文件夹的目录
        vasp_dir: 原始 vasp 文件所在目录
    
    返回:
        file_pairs: [(max_path, min_path, vasp_path, filename), ...]
    """
    file_pairs = []
    
    max_dir = os.path.join(split_dir, 'max')
    min_dir = os.path.join(split_dir, 'min')
    
    if not os.path.exists(max_dir):
        print(f"Error: max directory not found: {max_dir}")
        return file_pairs
    if not os.path.exists(min_dir):
        print(f"Error: min directory not found: {min_dir}")
        return file_pairs
    
    max_files = {f: os.path.join(max_dir, f) for f in os.listdir(max_dir) if f.endswith('.vasp')}
    min_files = {f: os.path.join(min_dir, f) for f in os.listdir(min_dir) if f.endswith('.vasp')}
    common_files = set(max_files.keys()) & set(min_files.keys())
    
    print(f"Found {len(max_files)} files in max folder")
    print(f"Found {len(min_files)} files in min folder")
    print(f"Found {len(common_files)} matching max/min pairs")
    
    matched_count = 0
    missing_vasp = []
    for filename in common_files:
        vasp_path = os.path.join(vasp_dir, filename)
        if os.path.exists(vasp_path):
            file_pairs.append((max_files[filename], min_files[filename], vasp_path, filename))
            matched_count += 1
        else:
            missing_vasp.append(filename)
    
    print(f"Found {matched_count} pairs with corresponding vasp files")
    if len(missing_vasp) > 0:
        print(f"Warning: {len(missing_vasp)} files missing in vasp directory")
        if len(missing_vasp) <= 5:
            for f in missing_vasp:
                print(f"  - {f}")
    
    return file_pairs

def process_file_pair_unlabeled(max_path, min_path, vasp_path, filename):
    """
    处理一对 max/min VASP 文件和对应的原始vasp文件（无标签版本）
    返回: (combined_features, filename) 或 None
    ...
    try:
        ...
        return combined_features, filename
        
    except Exception as e:
        print(f"Error processing {filename}: {e}")
        return None

def process_dataset_unlabeled(split_dir, vasp_dir, output_csv_path):
    """
    处理无标签数据集，共38个特征，并保存为CSV文件
    ...
    """
    file_pairs = find_matching_files_unlabeled(split_dir, vasp_dir)
    ...
    for max_path, min_path, vasp_path, filename in tqdm(file_pairs, desc="Processing file pairs"):
        result = process_file_pair_unlabeled(max_path, min_path, vasp_path, filename)
        if result is not None:
            combined_features, filename = result
    ...
    # 列顺序：filename + 38个特征（无label列）
    cols = ['filename'] + feature_names
    df = df[cols]
...
if __name__ == "__main__":
    # 配置路径 - 请根据实际情况修改
    split_dir = "data/split/sur_last"  # 包含 max 和 min 子文件夹的目录
    vasp_dir = "data/sur_last"     # 原始 vasp 文件所在目录
    output_csv_path = "data/dataset/features_38_unlabeled.csv"
    
    print("="*70)
    print("开始处理无标签数据集（38个特征）...")
    ...
    df = process_dataset_unlabeled(split_dir, vasp_dir, output_csv_path)
    
    if df is not None:
        print("\n" + "="*70)
        print("数据预处理完成!")
        print(f"数据集形状: {df.shape}")
        print(f"特征数量: {df.shape[1] - 1}")  # 减去filename列
